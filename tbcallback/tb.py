...
class TensorBoardFastAI(LearnerCallback):
    def __init__(self, writer, learn, track_weight=False, track_grad=False, show_images=False, metric_names=['Validation loss', 'Accuracy']):
        """
    ...
    def on_epoch_end(self, **kwargs:Any):
        ...
        if kwargs['last_metrics'][0] != None:
            ...
            if self.show_images:
                ...
                self.writer.add_image(tag='image', img_tensor=interp.plot_top_losses(9, figsize=(7,7)), global_step=self.delta_epochs)
